...
1. **Restaurant Performance** — top 10 by rating, count by city, price range distribution
2. **Order & Revenue** — monthly revenue trend, payment method split, cancellation rate
3. **Customer Behavior** — RFM-style segmentation, retention (new vs repeat), LTV ranking
4. **Review Analysis** — platform vs review rating comparison, rating distribution
5. **Delivery Performance** — avg/min/max delivery time, late delivery count by city
6. **Advanced Window Functions** — `RANK()`, `DENSE_RANK()`, `LAG()`, running totals
...
Follow the step-by-step instructions in [`excel_powerbi_guide.md`](excel_powerbi_guide.md).
...
This project uses a synthetic dataset generated to mirror the structure of the publicly available [Zomato Restaurants Dataset on Kaggle](https://www.kaggle.com/datasets/shrutimehta/zomato-restaurants-data). You can swap in the real CSV by replacing the data generation block in `01_data_cleaning.py` with:
